...
- `VulnerabilityDetailDialog`: shows direct and transitive security findings with sources and references; the actions **Open in ...** and **References...** in the dialog's top action bar are controlled by the current row selection, and a right-click on the row opens a context menu in all columns except **References** with **Open in Maven Repository** and **References...**.
