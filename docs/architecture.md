...
- `TransitiveVulnerabilitiesView`: standalone `JBPanel` view listing all transitive dependencies with known vulnerabilities (GroupId, ArtifactId, version, vulnerability count) in a sortable table; the pure `collectTransitiveVulnerabilityRows` helper builds and sorts its rows. Designed to host additional actions in the future.
