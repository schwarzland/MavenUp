...
- `MavenUpWindowFactory`: tool window factory and UI interaction for the table, update, and vulnerability workflows; the actions live in a top `ActionToolbar`, refresh data is collected via a non-blocking read action off the EDT. A central `JBTabbedPane` offers the main dependency table and the transitive vulnerabilities view as two tabs.
