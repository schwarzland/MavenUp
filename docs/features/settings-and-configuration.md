...
- **Configurable vulnerability cache**: Lets users choose the session-only vulnerability cache retention period and whether refreshes automatically rescan missing, expired, or invalidated entries; the defaults are four hours and automatic rescanning.
- **Vulnerability cache inspection**: Provides a read-only view of the current project's valid vulnerability-cache entries, including coordinates, scan sources, findings, scan timestamps, and expiry times.
