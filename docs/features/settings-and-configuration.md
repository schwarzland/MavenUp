...
- **Configurable vulnerability cache**: Lets users choose a 1–1440 minute session-only retention period and whether refreshes automatically rescan missing, expired, or invalidated entries; automatic rescanning becomes effective only after a successful manual scan.
- **Vulnerability cache inspection**: Provides a read-only, sortable view of the current session's valid vulnerability-cache entries, including coordinates, scan sources, findings, scan timestamps, and expiry times.
