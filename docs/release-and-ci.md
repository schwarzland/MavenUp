...
- **Dependency Review** (`.github/workflows/dependency-review.yml`): Runs on pull requests against `main` that touch the build or dependency configuration. It submits the dependency graph of the pull request branch and then compares it against `main` with `actions/dependency-review-action`. The job fails as soon as the pull request introduces a dependency with a known vulnerability of severity `moderate` or higher and posts a summary as a pull request comment in that case. Pull requests from forks are skipped because their `GITHUB_TOKEN` is read-only; they remain covered by the Dependabot alerts on `main`. The job is limited to 30 minutes.
