...
- **Version cache duration (minutes)**: How long a fetched version list stays cached per artifact (`groupId:artifactId`) before it is fetched from the repository again (default: 60). Applies to manual searches, background checks, and the automatic version lookup for vulnerable transitive dependencies. `0` disables the cache. Saving any setting on any page clears this cache immediately.
...
- **Vulnerability cache duration (minutes)**: How long a merged scan result stays cached per exact coordinate (`groupId:artifactId:version`), including coordinates with no findings, before it is queried live again (default: 1440, i.e. one day). Because the version is part of the cache key, applying an update automatically produces a fresh key for the affected coordinates. `0` disables the cache. Saving any setting on any page clears this cache immediately.
