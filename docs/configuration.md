...
- **Comment text**: The text written in front of the listed identifiers (default: `Pinned by MavenUp to fix:`). A trailing colon is dropped when no identifiers are listed; when the text is left empty, a generic `Added by MavenUp` comment is written. The field is disabled when **No comment** is selected.
- **Maximum number of identifiers**: Limits how many identifiers are listed (default: 3); the remaining ones are replaced with `and more`, for example `Pinned by MavenUp to fix: GHSA-1, GHSA-2, GHSA-3 and more`. Use `0` for no limit. The field is only enabled for the states that list identifiers.
