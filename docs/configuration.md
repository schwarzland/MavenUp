...
- **Version cache duration (minutes)**: How long a fetched version list stays cached per artifact (`groupId:artifactId`) before it is fetched from the repository again (default: 60). Applies to manual searches, background checks, and the automatic version lookup for vulnerable transitive dependencies. `0` disables the cache. Changing private GroupId prefixes or the Central-first lookup setting clears this cache because either setting can change repository results.
