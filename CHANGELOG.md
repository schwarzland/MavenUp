...
- The **New Version** column now shows a status icon and color-coded text: a green checkmark with green text when the dependency is up to date (current version equals the highest known version), or an orange arrow-up icon with orange text when a newer version is available; hovering over the cell shows a tooltip with the status details. The icon and color always reflect the currently **selected** version in the dropdown; when a version different from the current one is selected, the dropdown text is displayed in bold to indicate a pending change.
