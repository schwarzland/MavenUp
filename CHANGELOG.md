...
- Added a toolbar in the **Dependency Hierarchy** dialog with **Expand All**, **Collapse All**, **Navigate to pom.xml**, and **Show in Table** actions for quick tree navigation and jumping to the component in the main table.
- Added a **Show in Table** action to the toolbar and context menu of the **Dependency Hierarchy** dialog that closes the dialog, resets all filters, and selects the matching row in the main dependencies table.
...
- Added a **More Settings** group with quick links from the **MavenUp** root page to each sub-page, including a short description of what the page contains.
...
- Ordered the identifiers in the pom.xml vulnerability comment by severity, most severe first, so a truncated list keeps the most critical findings.
