...
- Grouped the two highest-version actions ("Select Highest Major Version", "Select Highest Minor Version") into a "Select Highest Version" toolbar dropdown (upload icon) whose tooltip clarifies that only the currently visible dependencies are changed, and kept "Reset All to Current Versions" as a standalone toolbar action whose tooltip states it applies to all dependencies regardless of active filters.
