...
- Added an "Explanatory comment when pinning a dependency to fix a vulnerability" setting that selects which identifiers the XML comment lists — no comment, advisory IDs (e.g. GHSA, default), aliases (e.g. CVE), or all identifiers — placed in a new **Pom.xml Changes** settings group alongside the relocated "Sync Maven changes after update" setting.
