...
- **Dependency table filtering**: A filter row above the main table combines a text field that filters by GroupId, ArtifactId or Property (case-insensitive substring match) with four combo boxes filtering by dependency type, pending version changes (All / With changes / Without changes), available updates (All / Update available / Up to date), and security findings (All / Vulnerable / Not vulnerable); the combo box options use self-describing, context-specific wording so the selected value is understandable on its own. Only rows matching all active filters remain visible. The changes combo box is enabled only once at least one row has a version selection differing from its current version; the updates combo box is enabled only after a successful "Scan new Versions" run and then shows only rows for which a newer version is available; the vulnerabilities combo box is enabled only after a successful "Scan for Vulnerabilities" run. Each control carries a tooltip explaining its effect, and a reset button at the end of the row clears all filters at once and is disabled while no filter is active.
- **Column-header sorting**: Clicking a column header cycles the dependency table through ascending, descending, and the original pom.xml order; the Current Version, Vulnerabilities, and New Version columns stay unsorted.
...
- **Bulk version selection**: Toolbar actions apply a target version to every loaded dependency at once — the highest available version (across major lines), the highest version within each dependency's current major line, or a reset that discards all selections and restores the current versions.
