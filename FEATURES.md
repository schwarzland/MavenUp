...
- **Two-tier action toolbar**: Tool window and Vulnerability Details dialog present their actions in a top IntelliJ `ActionToolbar`; core actions (Refresh, Check for Updates, Check Vulnerabilities, Update) are separated from the selection-dependent actions (Open in Repository, Vulnerability Details / References) by a separator, with Settings placed at the end. Actions render either as icon-only buttons (default, with tooltips) or as buttons with text labels, configurable in the settings.
...
- **Configurable toolbar button style**: Lets users switch the tool window and Vulnerability Details dialog toolbars between icon-only buttons (default) and buttons with text labels; the change applies immediately to the open tool window.
