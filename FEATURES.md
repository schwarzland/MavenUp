...
- **Safe update workflow**: Shows a confirmation dialog before applying changes to `pom.xml`, with an optional **Sync Maven Changes after update** checkbox (enabled by default) to automatically trigger the IDE's Maven sync after the update.
