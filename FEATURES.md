...
- **Two-tier action toolbar**: Tool window and Vulnerability Details dialog present their actions in a top IntelliJ `ActionToolbar`; core actions (Refresh, Find New Versions, Scan for Vulnerabilities, Update) are separated by a separator from a **Select Highest Version** dropdown that groups the two highest-version actions (Select Highest Major Version, Select Highest Minor Version) and a standalone Reset to Current Versions action, followed by the selection-dependent actions (Open in Repository, Vulnerability Details / References), with Settings placed at the end. Actions render either as icon-only buttons (with tooltips) or as buttons with text labels (default), configurable in the settings; when text labels are enabled the buttons show shortened labels (e.g. "New Versions", "Scan", "Update", "Highest", "Reset", "Open", "Details") while their tooltips keep the full text so the toolbar stays compact on low screen resolutions.
