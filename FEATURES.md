...
- **Content-fitted column widths**: The main dependency table and the Vulnerability Details table trim their column widths to the actual content (bounded by a minimum and maximum) after being populated, so as much as possible stays readable without manual resizing.
