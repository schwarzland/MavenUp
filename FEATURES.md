...
- **Version status indicators**: The **New Version** column displays a green checkmark icon with green text when the selected version equals the highest known version, or an orange arrow-up icon with orange text otherwise; hovering shows a tooltip with status details. The icon and color always reflect the currently selected version. When a version different from the current one is selected, the dropdown text is displayed in bold to indicate a pending change. Colors adapt to Light and Dark themes.
