...
- **Bulk version selection**: Toolbar actions apply a target version at once — the highest available version (across major lines) and the highest version within each dependency's current major line apply only to the currently visible (non-filtered) rows and surface a tooltip hint when a filter hides entries, while a reset discards all selections and restores the current versions regardless of filtering.
