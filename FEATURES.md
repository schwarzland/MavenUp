...
- **Transitive vulnerabilities view**: A toolbar toggle switches the tool window from the main dependency table to a dedicated view that lists every transitive dependency with known vulnerabilities (GroupId, ArtifactId, resolved version) alongside a vulnerability count column with the same severity-based coloring and sorting as the main table; rows are pre-sorted by highest severity, clicking the count opens the vulnerability details dialog, and pressing the toggle again returns to the main table. The toggle is enabled only after a scan found transitive vulnerabilities, and the filter row is hidden while the view is active. The view is designed to host additional actions in the future.
