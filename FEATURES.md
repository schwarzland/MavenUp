...
- **Configurable Maven Repository Browser**: The repository browser used for opening artifact version pages can be selected under **Settings > Tools > MavenUp**. Three options are available: **MVN Repository** (default), **Maven Central Search**, and **Sonatype Central**. The selection applies to both the context menu in the main table and the Component column in the Vulnerability Details dialog.
