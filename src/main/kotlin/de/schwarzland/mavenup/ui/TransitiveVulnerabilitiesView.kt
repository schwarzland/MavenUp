...
internal const val TRANSITIVE_VERSION_COLUMN = 2
 ...
 * erforderlichen Bestandteile werden übersprungen. Die Zeilen werden absteigend nach höchstem
...
internal class TransitiveVulnerabilitiesView(private val project: Project) : JBPanel<JBPanel<*>>(BorderLayout()) {
    ...
    fun update(
        ...
        val rows = collectTransitiveVulnerabilityRows(advisoriesByCoordinate, transitiveCoordinates)
        tableModel.setRowCount(0)
        rows.forEach { row ->
            tableModel.addRow(arrayOf(row.groupId, row.artifactId, row.version, row.cell))
        }
