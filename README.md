...
- **Update-Check über Repositories**: Prüft verfügbare Versionen in Maven Central und konfigurierten privaten Repositories (`settings.xml`), unterstützt Authentifizierung, dedupliziert identische Repository-URLs, nutzt bei leerem Maven-User-Settings-Pfad automatisch `${user.home}/.m2/settings.xml` und bietet eine konfigurierbare Central-first-Strategie.
...
- **Central-first mit konfigurierbarem Short-Circuit**: Fragt Maven Central priorisiert zuerst ab; ist die Abfrage dort erfolgreich, können weitere private Repositories optional übersprungen werden (Standard: aktiv) oder weiterhin abgefragt werden.
...
10. **Adjust repository strategy**: In settings, **Stop after a successful Maven Central lookup** controls whether private repositories are skipped after a successful Central lookup (faster) or still queried afterwards (more complete for private-only versions).
