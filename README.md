...
- **Filter für die Dependency-Tabelle**: Oberhalb der Tabelle filtert ein Textfeld case-insensitiv über die Spalten **GroupId**, **ArtifactId** und **Property**; eine Combobox filtert zusätzlich über die Spalte **Type**. Beide Filter werden kombiniert, sodass nur noch passende Zeilen angezeigt werden. Die Combobox listet die aktuell vorhandenen Typen sowie die Option **All** (kein Typ-Filter).
...
9. **Filter**: Über der Tabelle grenzt das Textfeld die Anzeige case-insensitiv nach **GroupId**, **ArtifactId** oder **Property** ein; die Combobox filtert zusätzlich nach **Type**. Beide Filter wirken zusammen, sodass nur noch passende Zeilen sichtbar bleiben. Mit **All** wird der Typ-Filter aufgehoben.
...
9. **Filter**: The text field above the table narrows the view case-insensitively by **GroupId**, **ArtifactId** or **Property**; the combo box additionally filters by **Type**. Both filters combine so that only matching rows remain visible. Selecting **All** removes the type filter.
