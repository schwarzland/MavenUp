...
9. **Rechtsklick-Kontextmenü**: Ein Rechtsklick auf eine Zeile bietet **Filter by "..."** (nur beim Rechtsklick auf die Spalten **GroupId**, **ArtifactId** oder **Property** mit nicht-leerem Wert; setzt den angeklickten Wert als alleinigen Textfilter, ersetzt dabei einen bereits vorhandenen Filtertext und wendet ihn sofort an), **Navigate to pom.xml**, **Open in Maven Repository** (öffnet die aktuelle Version im konfigurierten Repository-Browser), **Set Highest Major Version** und **Set Highest Minor Version** (wählen ausschließlich für die angeklickte Dependency die höchste verfügbare Version über alle Major-Linien bzw. die höchste Version innerhalb der aktuellen Major-Linie; beide sind erst verfügbar, wenn die verfügbaren Versionen dieser Dependency bereits abgerufen wurden), **Reset to Current Version** (verwirft ausschließlich für die angeklickte Dependency die Auswahl und ist nur aktiv, wenn eine abweichende Version ausgewählt ist) und **Show Vulnerability Details** (öffnet den Dialog mit den gefundenen Sicherheitsbefunden; deaktiviert, wenn die Dependency keine Befunde hat). Alle Einträge bleiben stets sichtbar und ändern nur ihren Aktivierungszustand.
...
- **Confirm before resetting all version selections**: Legt fest, ob die Aktion **Reset All to Current Versions** vor dem Verwerfen aller Versionsauswahlen einen Bestätigungsdialog anzeigt (Standard: an). Der Dialog bietet eine Option „Don't ask again", die diese Einstellung ebenfalls deaktiviert. Diese Einstellung greift nur, wenn kein Filter aktiv ist; bei aktivem Filter wird stattdessen immer gefragt, ob auf alle oder nur auf die gefilterten Abhängigkeiten zurückgesetzt werden soll.
...
|---|---|---|---|
| [cvss-calculator](https://github.com/stevespringett/cvss-calculator) (us.springett) | 1.5.1 | [Apache License 2.0](https://www.apache.org/licenses/LICENSE-2.0) | CVSS score calculation for vulnerability findings |
