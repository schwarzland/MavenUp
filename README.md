...
9. **Rechtsklick-Kontextmenü**: Ein Rechtsklick auf eine Zeile bietet **Navigate to pom.xml**, **Open in Maven Repository** (öffnet die aktuelle Version im konfigurierten Repository-Browser), **Set Highest Major Version** und **Set Highest Minor Version** (wählen ausschließlich für die angeklickte Dependency die höchste verfügbare Version über alle Major-Linien bzw. die höchste Version innerhalb der aktuellen Major-Linie; beide sind erst verfügbar, wenn die verfügbaren Versionen dieser Dependency bereits abgerufen wurden), **Reset to Current Version** (verwirft ausschließlich für die angeklickte Dependency die Auswahl und ist nur aktiv, wenn eine abweichende Version ausgewählt ist) und ggf. **Show Vulnerability Details** (öffnet den Dialog mit den gefundenen Sicherheitsbefunden, falls vorhanden).
