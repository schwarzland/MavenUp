...
- **Update-Check über Repositories**: Prüft verfügbare Versionen in Maven Central und konfigurierten privaten Repositories (`settings.xml`), unterstützt Authentifizierung, dedupliziert identische Repository-URLs und nutzt bei leerem Maven-User-Settings-Pfad automatisch `${user.home}/.m2/settings.xml`.
