...
- **TransitiveVulnerabilitiesView**: eigenständige `JBPanel`-Ansicht (Top-Level in `ui`), die alle
  ...
  Vulnerabilities (`clearVulnerabilities`) bzw. bei einem neuen Scan zurückgesetzt. Die Ansicht liegt als zweiter Tab
  **Transitive CVEs** in einer zentralen `JBTabbedPane` (`viewTabs`, Indizes `TAB_INDEX_MAIN_TABLE`/
  `TAB_INDEX_TRANSITIVE_VIEW`) neben dem Tab **Dependencies**; ein `ChangeListener` synchronisiert die Auswahl über
  `MavenUpWindowFactory.syncSelectedViewTab` nach `showingTransitiveView`, `setTransitiveViewVisible` wählt den Tab
  programmatisch. `updateTransitiveVulnerabilitiesView` schreibt die Anzahl betroffener Koordinaten
  (`transitiveVulnerabilityCount`/`hasTransitiveVulnerabilities`) in den Tab-Titel, aktiviert den Tab nur bei Funden und
  schaltet ohne Funde auf den Haupt-Tab zurück. Jeder Tab bringt seine eigene Filterzeile mit; die der Haupttabelle liegt
...
- **UI-Hilfsdateien (ui)**: Zustandslose Top-Level-Helfer, die aus `MavenUpWindowFactory.kt`
  ...
  - `MavenUpTableConstants.kt`: Spalten-Indizes und Message-Key-/Typ-Konstanten sowie die
    Tab-Indizes `TAB_INDEX_MAIN_TABLE`/`TAB_INDEX_TRANSITIVE_VIEW` der zentralen `JBTabbedPane`.
