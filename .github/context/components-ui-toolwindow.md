...
- `showVulnerabilityApiError` blendet eine fehlgeschlagene Vulnerability-API-Abfrage als schließbaren `InlineBanner` mit `EditorNotificationPanel.Status.Error` im `vulnerabilityApiErrorPanel` direkt oberhalb der Tabelle ein, statt eines blockierenden Dialogs; die Banner-Aktion **Open Settings** ruft `openVulnerabilityCheckSettings()` auf, springt damit direkt zur Einstellungs-Unterseite **Vulnerability Check** (`MavenUpVulnerabilityConfigurable`) und schließt den Hinweis. `hideVulnerabilityApiError` entfernt ihn beim Schließen, über die Banner-Aktion, bei einem Refresh mit `clearVulnerabilities` und wenn ein Folge-Scan ohne Fehler abschließt. `performVulnerabilityCheck` kombiniert dazu Fehler aus beiden Quellen: den OSS-Index-Fehler (`OssIndexScanResult.errorMessage`, z. B. fehlendes/ungültiges Token) und einen über den `onError`-Callback von `VulnerabilityApiService.fetchVulnerabilityAdvisories` gemeldeten OSV.dev-Fehler (fehlgeschlagene HTTP-Antwort oder Exception); liegen beide vor, werden sie zeilenweise in einem gemeinsamen Banner angezeigt.
