...
- `showOssIndexError` blendet eine fehlgeschlagene OSS-Index-Abfrage (fehlendes/ungültiges Token oder sonstiger Fehler, aus `OssIndexScanResult.errorMessage`) als schließbaren `InlineBanner` mit `EditorNotificationPanel.Status.Error` im `ossIndexErrorPanel` direkt oberhalb der Tabelle ein, statt eines blockierenden Dialogs; die Banner-Aktion **Open Settings** ruft `openVulnerabilityCheckSettings()` auf, springt damit direkt zur Einstellungs-Unterseite **Vulnerability Check** (`MavenUpVulnerabilityConfigurable`) und schließt den Hinweis. `hideOssIndexError` entfernt ihn beim Schließen, über die Banner-Aktion und bei einem Refresh mit `clearVulnerabilities`.
