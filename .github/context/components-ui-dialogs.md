...
- **VersionCacheContentsDialog** / **VulnerabilityCacheContentsDialog**: Diagnosedialoge (`internal class`, **Close** zum Schließen und eine unmittelbar wirksame **Invalidate**-Aktion im Kotlin-UI-DSL-v2-Inhaltsbereich), die über die Buttons **Show Cache Contents...** auf den jeweiligen Einstellungsseiten `MavenUpVersionsConfigurable` bzw. `MavenUpVulnerabilityConfigurable` geöffnet werden. Zeigen eine sortierbare `JBTable`, gebaut aus einem frischen `snapshot()` der injizierbaren `VersionMetadataCache`/`VulnerabilityResultCache`-Instanzen (Standard: `getInstance()`): „Version Cache Contents" (Spalten GroupId, ArtifactId, Anzahl zwischengespeicherter Versionen, Zeitstempel, TTL) und „Vulnerability Cache Contents" (Spalten Koordinate, Anzahl zwischengespeicherter Funde, Zeitstempel, TTL). `remainingCacheTtlSeconds` berechnet aus Zeitstempel und aktueller Einstellung die verbleibende TTL am Schnappschusszeitpunkt, auf ganze Sekunden aufgerundet; abgelaufene Einträge und deaktivierte Caches zeigen 0. Es gibt keinen laufenden Countdown. `invalidateCache()` leert den anwendungsweiten Cache des jeweiligen Dialogs über `clear()`; `refreshTable()` erneuert anschließend die Tabelle, ohne einen Scan oder eine Suche auszulösen. Beide Tabellen nutzen denselben Sortierzyklus (aufsteigend → absteigend → unsortiert) und dieselben `sortableHeaderIcon`-Indikatoren wie die Haupttabelle (`installSortableHeaderRenderer`, eigener `buildRowSorter`); die Anzahl- und TTL-Spalten sortieren numerisch über einen expliziten `numericColumns`-Parameter (statt über `getColumnClass`, das bei `DefaultTableModel` stets `Object` liefert), alle übrigen Spalten über `cellTextComparator`. Spaltenbreiten werden über `trimColumnWidthsToContent` inhaltsbasiert getrimmt, Zeilenhöhe über `applyRecommendedRowHeight` gesetzt. Der Top-Level-Helfer `formatCacheTimestamp` formatiert den Abfrage-Zeitstempel als `yyyy-MM-dd HH:mm:ss`, damit die Zeitstempel-Spalte auch als reiner Text lexikografisch korrekt sortiert.
- **DependencyHierarchyPanel**: einbettbares `JBPanel` (Top-Level in `ui`), das über einen `OnePixelSplitter` als Split-View rechts neben der Haupttabelle bzw. der transitiven CVE-Tabelle eingeblendet wird und den vollständigen Einbindungs- und Management-Hierarchiebaum sowie alle transitiven Unterabhängigkeiten einer ausgewählten Koordinate darstellt. Die Split-View fungiert als reaktiver Master-Detail-Inspector: Im geöffneten Zustand synchronisiert sie sich sofort mit Zeilenselektionen für alle Tabelleneinträge (Dependencies, Plugins, Managed Entries, Parent); wird eine Zeile ohne Koordinaten oder keine Zeile gewählt, zeigt `showEmpty()` einen informativen Empty State, ohne das Panel zuzuklappen. Baut über `DependencyHierarchyService.buildHierarchy` das Datenmodell auf und visualisiert es in einem interaktiven `Tree` (`DefaultTreeModel`, `expandAllNodes`) innerhalb eines `JBScrollPane` in einem Kotlin UI DSL v2-Panel. `TreeSpeedSearch` ermöglicht die Schnellsuche nach GroupId, ArtifactId oder Version. `DependencyHierarchyTreeCellRenderer` formatiert die Baumknoten mit typspezifischen Icons (`AllIcons.Nodes.*`), Präfix-Kennzeichnungen (`[Parent POM]`, `[BOM Import]`, `[Dependency Management]`, `[Plugin Management]`, `[DD]` für direkte Dependencies und Plugins, `[TD]`), hebt die Ziel-Abhängigkeit bzw. das Ziel-Plugin farblich hervor (`TARGET_DEPENDENCY_COLOR`, passt sich über `JBColor` an Light- und Dark-Mode an), formatiert Versionen in Fettschrift und zeigt Zusatzinformationen (Property-Namen, Scopes, `MANAGED`-Marker). Der Toolbar-Toggle **Show GroupIds** blendet die Group-ID aus allen Knoten aus oder wieder ein, ohne den Hierarchiebaum neu aufzubauen; die Einstellung bleibt bei Auswahlwechseln im Panel erhalten. Transitive Knoten, deren Koordinate weder in der Haupttabelle noch in **Transitive CVEs** vorliegt, erhalten über `isDependencyInTable` ausgegraute Textattribute und ein deaktiviertes Icon; Zielknoten und verwundbare Transitive behalten Vorrang. Nach einem Vulnerability-Scan werden vulnerable transitive Abhängigkeiten über `vulnerabilityAdvisoriesProvider` gesondert mit dem Warn-Icon `AllIcons.General.BalloonWarning`, Fehlerfarben (`VULNERABLE_TEXT_COLOR`), `VULNERABLE`-Status/Schweregrad im Detailtext und erklärendem Tooltip hervorgehoben. Eine eigene Toolbar bietet **Expand All**, **Collapse All**, **Show GroupIds**, **Navigate to pom.xml**, abhängig vom tatsächlichen Sprungziel **Select in Dependencies** oder **Select in Transitive CVEs** und einen Schließen-Button (**Close** zum Zuklappen des Seitenpanels; Schließen auch per `Escape`-Taste möglich). Ein Rechtsklick öffnet ein Kontextmenü (**Navigate to pom.xml** und die zielabhängig benannte Auswahlaktion), zusätzlich kann per `Enter` oder `F4` (`navigateToSelectedNode`) per PSI-Navigation (`PomNavigationService.navigateToDependency` bzw. `OpenFileDescriptor`) punktgenau zur deklarierenden Stelle in der jeweiligen `pom.xml` gesprungen werden. Die Navigationsaktion **Navigate to pom.xml** ist deaktiviert (`canNavigateToSelectedNode`), wenn ein Modul ([Module]/`PROJECT`) ausgewählt ist oder die Deklaration nicht in einer `pom.xml` des Projekts liegt; die Auswahlaktion ist aktiv (`canNavigateToTable`), wenn die Koordinate in mindestens einer der Tabellen existiert. `createToolbar` setzt `targetComponent` bewusst auf das Panel selbst statt auf den jeweiligen `Tree`: In den Empty-State-Zweigen (`showEmpty`, sowie der leere Zweig von `showHierarchy` ohne Kindknoten) wird der erzeugte (leere) Baum nie in die Komponentenhierarchie eingehängt und wäre daher nie „showing"; `ActionManagerImpl` verweigert sonst jede Toolbar-Aktion (inklusive **Close**) mit der Warnung „target component is not showing". Das Panel selbst ist dagegen immer sichtbar, solange die Split-View geöffnet ist.

  Für die zielabhängig beschriftete Auswahlaktion prüft `MyToolWindow` beide Tabellen: Existiert die ausgewählte
  Koordinate in **Dependencies**, trägt die Aktion den Namen **Select in Dependencies** und selektiert dort die
  Zeile; nur ausschließlich durch den Scan bekannte Koordinaten verwenden **Select in Transitive CVEs**.
...
- **MavenUpVersionsConfigurable**: Unterseite **Versions and Updates**, gegliedert in die Gruppen
  **Version Lookup** (`autoSearchVersions`, `stopAfterCentralSuccess`, `versionCacheTtlMinutes` mit Button `showVersionCacheButton` zum Öffnen von `VersionCacheContentsDialog`), **Privacy** (`privateGroupIds`) und
...
- **MavenUpVulnerabilityConfigurable**: Unterseite **Vulnerability Check** mit `checkTransitiveDependencies`,
  `vulnerabilityCacheTtlMinutes` (mit Button `showVulnerabilityCacheButton` zum Öffnen von `VulnerabilityCacheContentsDialog`)
