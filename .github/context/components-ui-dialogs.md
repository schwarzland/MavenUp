...
- **DependencyHierarchyDialog**: modaler `DialogWrapper` (Top-Level in `ui`), der den vollständigen Einbindungs- und Management-Hierarchiebaum einer ausgewählten Koordinate darstellt. Die Aktion ist in der Haupttabelle für Managed Dependencies und Managed Plugins und in der Ansicht **Transitive CVEs** für jede Koordinate verfügbar. Rein informativer Dialog – zeigt ausschließlich einen **Close**-Button (kein OK/Cancel) gemäß den JetBrains UI-Richtlinien. Baut über `DependencyHierarchyService.buildHierarchy` das Datenmodell auf und visualisiert es in einem interaktiven `Tree` (`DefaultTreeModel`, `expandAllNodes`) innerhalb eines `JBScrollPane` in einem Kotlin UI DSL v2-Panel. `TreeSpeedSearch` ermöglicht die Schnellsuche nach GroupId, ArtifactId oder Version. `DependencyHierarchyTreeCellRenderer` formatiert die Baumknoten mit typspezifischen Icons (`AllIcons.Nodes.*`), Präfix-Kennzeichnungen (`[Parent POM]`, `[BOM Import]`, `[Dependency Management]`, `[Plugin Management]`, `[Direct Dependency]`, `[Direct Plugin]`, `[Transitive Dependency]`), hebt die Ziel-Abhängigkeit bzw. das Ziel-Plugin farblich hervor (`TARGET_DEPENDENCY_COLOR`, passt sich über `JBColor` an Light- und Dark-Mode an), formatiert Versionen in Fettschrift und zeigt Zusatzinformationen (Property-Namen, Scopes, `MANAGED`-Marker). Eine Toolbar bietet **Expand All**, **Collapse All**, **Navigate to pom.xml** und **Select in Table** (schließt den Dialog, setzt alle Filter zurück und selektiert die Koordinate in der Haupttabelle). Ein Rechtsklick öffnet ein Kontextmenü (**Navigate to pom.xml** und **Show in Table**), zusätzlich kann per `Enter` oder `F4` (`navigateToSelectedNode`) per PSI-Navigation (`PomNavigationService.navigateToDependency` bzw. `OpenFileDescriptor`) punktgenau zur deklarierenden Stelle in der jeweiligen `pom.xml` gesprungen werden. Die Navigationsaktion **Navigate to pom.xml** ist deaktiviert (`canNavigateToSelectedNode`), wenn ein Modul ([Module]/`PROJECT`) ausgewählt ist oder die Komponente nicht in einer `pom.xml` des Projekts deklariert ist (transitive Abhängigkeiten sind nur navigierbar, wenn sie im `dependencyManagement` als verwaltet deklariert sind); **Show in Table** ist deaktiviert (`canNavigateToTable`), wenn die Koordinate nicht in der Haupttabelle enthalten oder ein Modulknoten ist.
